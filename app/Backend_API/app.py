...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "mysql" " mysqlconnector://root@localhost:3306/cf1"
)
...
class employee(db.Model):
    ...
    def get_designation(self):
        return {"currentDesignation": self.currentDesignation}
...
class cohort(db.Model):
    ...
    courseName = db.Column(
        db.String(100), db.ForeignKey(course.courseName), primary_key=True
    )
    ...
    trainerName = db.Column(
        db.String(100), db.ForeignKey(employee.employeeName), nullable=False
    )
...
class badges(db.Model):
    ...
    employeeName = db.Column(
        db.String(100), db.ForeignKey(employee.employeeName), primary_key=True
    )
    badges = db.Column(
        db.String(100),
    ...
    def get_badges_cohort(self):
        return {"badges": self.badges, "cohortName": self.cohortName}
...
class enrollment(db.Model):
    ...
    courseNameEnrolled = db.Column(
        db.String(100),
        db.ForeignKey(cohort.courseName),
        nullable=False,
        primary_key=True,
    )
    cohortNameEnrolled = db.Column(
        db.String(100),
        db.ForeignKey(cohort.cohortName),
        nullable=False,
        primary_key=True,
...
class enrollmentRequest(db.Model):
    ...
    courseNameRequest = db.Column(
        db.String(100),
        db.ForeignKey(cohort.courseName),
        nullable=False,
        primary_key=True,
    )
    cohortNameRequest = db.Column(
        db.String(100),
        db.ForeignKey(cohort.cohortName),
        nullable=False,
        primary_key=True,
    )
    learnerName = db.Column(
        db.String(100),
...
class userAttempt(db.Model):
    ...
    courseName = db.Column(
        db.String(100), db.ForeignKey(question.courseName), primary_key=True
    )
    cohortName = db.Column(
        db.String(100), db.ForeignKey(question.cohortName), primary_key=True
    )
    chapterID = db.Column(
        db.Integer, db.ForeignKey(question.chapterID), primary_key=True
    )
    questionID = db.Column(
        db.Integer, db.ForeignKey(question.questionID), primary_key=True
    )
...
class materialStatus(materials):
    ...
    def __init__(
        self, courseName, cohortName, chapterID, materialID, employeeName, done
    ):
        self.courseName = courseName
        self.cohortName = cohortName
        self.chapterID = chapterID
...
# CF3 start
@app.route(
    "/viewMaterials/<string:courseName>/<string:cohortName>/<string:employeeName>"
)
def viewMaterials(courseName, cohortName, employeeName):
    try:
        ...
        return jsonify({"code": 200, "materials": output}), 200
...
@app.route(
    "/updateMaterialStatus/<string:courseName>/<string:cohortName>/<string:chapterID>/<string:materialID>/<string:employeeName>"
)
def updateMaterialStatus(courseName, cohortName, chapterID, materialID, employeeName):
    try:
        ...
        return (
            jsonify({"code": 200, "message": "Successfully updated material status."}),
            200,
        )
...
@app.route(
    "/completedCourse/<string:courseName>/<string:cohortName>/<string:employeeName>"
)
def completedCourse(courseName, cohortName, employeeName):
    ...
    try:
        ...
        # add to badges
        badge = badges(
            employeeName=employeeName, badges=courseName, cohortName=cohortName
        )
        ...
        return (
            jsonify({"code": 404, "message": "Error occured while adding new badge"}),
            404,
        )
...
def recordAttempt():
    ...
    try:
        ...
        return (
            jsonify({"code": 201, "message": "Successfully recorded user attempt"}),
            201,
        )
        ...
        return (
            jsonify(
                {"code": 404, "message": "Error occured while adding new user attempt"}
            ),
            404,
        )
...
@app.route(
    "/retrieveQuizResult/<string:courseName>/<string:cohortName>/<string:chapterID>/<string:employeeName>"
)
def retrieveQuizResult(courseName, cohortName, chapterID, employeeName):
    ...
    try:
        ...
        return jsonify({"code": 200, "quiz_result": chapter_content}), 200

    except:
        return (
            jsonify(
                {"code": 404, "message": "Error occured while retrieving quiz result"}
            ),
            404,
        )
...
def getCurrentDesignation(employeeName):
    ...
    if result:
        return jsonify({"code": 200, "data": result.get_designation()}), 200

    return jsonify({"code": 404, "message": "EmployeeName does not exist"}), 404
...
def viewAllCourses():
    ...
    return (
        jsonify({"code": 404, "message": "Error occured while retrieving all courses"}),
        404,
    )
...
def viewAllBadges(employeeName):
    ...
    if result:
        return jsonify(
            {"code": 200, "badges": [element.get_badges() for element in result]}
        )

    return jsonify({"code": 404, "message": "Invalid employeeName"}), 404
            ...
            jsonify(
                {
                    "code": 200,
                    "badges_cohort": [
                        element.get_badges_cohort() for element in result
                    ],
                }
            ),
            200,
...
def viewAllEnrolledCourses(employeeName):
    ...
    if result:
        ...
        return jsonify({"code": 200, "enrollments": output}), 200
...
def viewAllRequests(learnerName):
    ...
    if result:
        ...
        return jsonify({"code": 200, "requests": output}), 200

    return jsonify({"code": 404, "message": "Invalid learnerName"}), 404
...
def adminViewAllRequests():
    ...
    if result:
        ...
        return jsonify({"code": 200, "requests": result}), 200
...
@app.route(
    "/delete/<string:learnerName>/<string:courseNameRequest>/<string:cohortNameRequest>"
)
def delete_request(learnerName, courseNameRequest, cohortNameRequest):

    request = enrollmentRequest.query.filter_by(
        learnerName=learnerName,
        courseNameRequest=courseNameRequest,
        cohortNameRequest=cohortNameRequest,
    ).first()
    ...
    if request:
        try:
            ...
            return jsonify({"code": 200, "message": "Request Deleted Successfully"})
    ...
    return jsonify({"code": 404, "message": "Enrollment request does not exist"}), 404
...
def viewAllCohort(courseName):
    ...
    if result:
        return (
            jsonify(
                {"code": 200, "cohorts": [element.get_dict() for element in result]}
            ),
            200,
        )

    return jsonify({"code": 404, "message": "Invalid courseName"}), 404


# approve a learner's request (admin view)
@app.route(
    "/processRequest/<string:learnerName>/<string:courseName>/<string:cohortName>"
)
    ...
    if result:
        ...
        if slotLeft > 0:
            try:
                ...
                return (
                    jsonify(
                        {"code": 200, "message": "Successfully processed request."}
                    ),
                    200,
                )
...
@app.route(
    "/self_enrol_request/<string:courseName>/<string:cohortName>/<string:learnerName>"
)
def self_enrol_request(courseName, cohortName, learnerName):
    ...
    try:
        ...
        return (
            jsonify(
                {"code": 200, "message": "Enrollment request created successfully."}
            ),
            200,
        )
...
@app.route(
    "/setEnrollmentPeriod/<string:courseName>/<string:cohortName>/<string:enrollmentStartDate>/<string:enrollmentStartTime>/<string:enrollmentEndDate>/<string:enrollmentEndTime>"
)
def setEnrollmentPeriod(
    courseName,
    cohortName,
    enrollmentStartDate,
    enrollmentStartTime,
    ...
    if result:
        try:
            ...
            return (
                jsonify(
                    {"code": 200, "message": "Successfully set enrollment period."}
                ),
                200,
            )
    ...
    return (
        jsonify(
            {"code": 404, "message": "Error occured while setting enrollment period."}
        ),
        404,
    )
...
def retrieveQualifiedLearners(courseName, cohortName):
    try:
        ...
        return jsonify({"code": 200, "QualifiedLearners": output}), 200
...
def assignLearners():
    ...
    if slotLeft < number_learners:
        return jsonify({"code": 404, "message": "Insufficient number of slots"}), 404
    ...
    try:
        ...
        return (
            jsonify(
                {"code": 201, "message": "Sucessfully assigned learners to course"}
            ),
            201,
        )
        ...
        return (
            jsonify(
                {"code": 404, "message": "An error occurred while assigning learners."}
            ),
            404,
        )
            ...
            jsonify(
                {
                    "code": 200,
                    "EnrolledLearners": [
                        element.get_employeeName() for element in result
                    ],
                }
            ),
            200,
...
def getAllChapters(courseName, cohortName):
    ...
    if output:
        return jsonify({"code": 200, "chapters": output}), 200

    return (
        jsonify({"code": 404, "message": "Error occured while retrieving chapters"}),
        404,
    )
...
def createNewChapter():
    ...
    try:
        ...
        return (
            jsonify(
                {"code": 404, "message": "Error occured while creating new chapter"}
            ),
            404,
        )
...
def createNewQuiz():
    ...
    try:
        ...
        return (
            jsonify({"code": 404, "message": "Invalid courseName and cohortName"}),
            404,
        )
    ...
    try:
        ...
        return jsonify({"code": 200, "message": "Quiz successfully created"})

    except:
        return jsonify(
            {"code": 404, "message": "Error occured while creating questions"}
        )
...
def viewQuiz(courseName, cohortName, chapterID):
    try:
        ...
        return jsonify({"code": 200, "chapter_content": chapter_content}), 200

    except:
        return (
            jsonify({"code": 404, "message": "Error occurred while viewing quiz"}),
            404,
        )
