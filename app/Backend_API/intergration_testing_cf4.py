...
import json
from app import app, db, chapter, question, options
...
class TestCreateNewChapter(TestApp):
    def test_createNewChapter(self):
        ...
        #get chapters 
        chapter1 = chapter.query.filter_by(courseName = "Introduction to life", cohortName = "G1").first()
...
class TestDeleteQuiz(TestApp):
    def test_deleteQuiz(self):
        ...
        question_aft = question.query.filter_by(courseName = "Introduction to life", cohortName = "G1", chapterID = -1).first()
        self.assertIsNone(question_aft)

        # check if options are still there -- should be empty
        option_aft = options.query.filter_by(courseName = "Introduction to life", cohortName = "G1", chapterID = -1, questionID = 1).first()
        self.assertIsNone(option_aft)

        # check if chapter is still there -- should be there
        chapter_aft = chapter.query.filter_by(courseName = "Introduction to life", cohortName = "G1").first()
